...
;;;  (IF (NULL rinfpiede) (setq rinfpiede (carica "rinfpiede" modalita) rinfpied $tb$))
  ...
  (WHILE (SETQ ENT (SSNAME GR NR))
	...
	(if (= forisn "Si")
	 ...
	 (if (nth 4 rigeman)
	   (progn
	     (setq halt_f_r (+ halt (atof (rtos (nth 6 rigeman) 2 1))  (atof (rtos (nth 7 rigeman) 2 1))))
	     (setq lunglam_r (+ lung lung2 (atof (rtos (nth 4 rigeman) 2 1)) (atof (rtos (nth 5 rigeman) 2 1))))
	     (setq lunglam_el lunglam_r)
	     )
	   )
	 )
       )
